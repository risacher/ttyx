...
function readConfig(file) {
  ...
  if (file || options.config) {
    ...
    json = options.config || file;
    console.log(file, dir, json, options);
  } else {
    dir = process.env.TTYX_PATH || path.join(home, '.ttyx');
...
function checkConfig(conf) {
  ...
  conf.shell = conf.shell || (__dirname + '/../bin/ssh-localhost');

  // arguments & env to shell, if they exist
  conf.shellArgs = conf.shellArgs || [];
  conf.shellEnv = conf.shellEnv || [];
  ...
  conf.term.geometry = conf.term.geometry || [80, 30] ; 
  ...
  conf.term.fontSize = 12; // 12
  conf.term.fontFamily = 'Menlo, Consolas, "DejaVu Sans Mono", monospace';
...
function checkLegacy(conf) {
  ...
  if (conf.userScript) {
    out.push(''
      ...
      + '`user.js` in `~/.ttyx/static/user.js` instead.');
  }

  if (conf.userStylesheet) {
    out.push(''
      + '`userStylesheet` is deprecated, please '
      + 'place `user.js` in `~/.ttyx/static/user.js` instead.');
  }

  if (conf.stylesheet) {
    out.push(''
      + '`stylesheet` is deprecated, please place '
      + '`user.css` in `~/.ttyx/static/user.css` instead.');
  }

  if (conf.hooks) {
    out.push(''
      + '`hooks` is deprecated, please programmatically '
      + 'hook into your ttyx server instead.');
  }
...
function killall() {
  ...
  spawn('/bin/sh',
    ['-c', 'kill $(ps ax | grep -v grep | grep ttyx | awk \'{print $1}\')'],
    options);
